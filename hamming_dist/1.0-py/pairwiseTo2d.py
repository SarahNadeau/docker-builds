...
def parseArgs():
    parser = ArgumentParser(
        description="Converts a list of pairwise values " "into an 2-dimensional array",
        add_help=False,
        epilog="NOTE: data after " "the third column are ignored",
    )
    ...
    req.add_argument(
        ...
        metavar="FILE",
        type=str,
        help="input file with 3 columns (<id1> <id2> <val>)",
    )
    ...
    opt.add_argument(
        ...
        action="store_false",
        default=True,
        help="pairwise values are directional, so "
    ...
    opt.add_argument(
        ...
        action="store_true",
        default=False,
        help="apply alphabetical sorting of <id1> and <id2> "
    ...
    opt.add_argument(
        ...
        metavar="FILE",
        default=None,
        help="2-dimensional output matrix [stdout]",
    )
    opt.add_argument(
        "--in-delim",
        metavar="STR",
        type=str,
        default="\t",
        help="input file data delimiter ['\\t']",
    )
    opt.add_argument(
        "--out-delim",
        metavar="STR",
        type=str,
        default="\t",
        help="output file data delimiter ['\\t']",
    )
...
def main():
    ...
    with open(ifh, "r") as ifh:
        for ln in ifh:
            ...
            if len(l) < 3:
                sys.stderr.write(
                    "ERROR: unable to parse first three columns",
                    "from:\n{}\n".format(ln),
                )
                sys.exit(1)
            dat[(l[0], l[1])] = l[2]
